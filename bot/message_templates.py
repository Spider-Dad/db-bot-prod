...
def get_welcome_message(is_admin: bool = False, is_authorized: bool = False) -> str:
    ...
    if not is_authorized:
        return "👋 Привет! Ожидай приглашения от администратора для добавления тебя в систему уведомлений о событиях нашей службы.Пожалуйста, установи @username, если его у тебя ещё нет."
...
Ты был добавлен в систему в чат-бот уведомлений нашей службы. Для получения уведомлений требуется твое согласие.
Чтобы подтвердить согласие на получение уведомлений, пожалуйста, нажми /start в чате с ботом."""
