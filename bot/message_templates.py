...
def format_birthday_reminder(template: str, first_name: str, last_name: str, 
    ...
    """Форматирование сообщения с напоминанием о дне рождения с поддержкой HTML"""
    try:
...
def get_welcome_message(is_admin: bool = False, is_authorized: bool = False) -> str:
    ...
    if not is_authorized:
        return "❌ У вас нет доступа к боту. Ожидайте приглашения от администратора для добавления вас в систему.Пожалуйста, убедитесь, что у вас установлен @username."

    # Базовое приветствие для всех авторизованных пользователей
    base_message = """<b>Привет!</b> 👋
Я бот для напоминания о событиях нашей службы"""
...
Вы были добавлены в систему напоминаний о днях рождения. Для получения уведомлений требуется ваше согласие.
    
Чтобы подтвердить согласие на получение уведомлений, пожалуйста, нажмите /start в чате с ботом.
    
Если вы не хотите получать уведомления, просто проигнорируйте это сообщение."""
...
• {phone_pay} - Номер телефона для перевода (для изменения обратитесь к @spiderdad)
• {name_pay} - Имя получателя перевода (для изменения обратитесь к @spiderdad)
